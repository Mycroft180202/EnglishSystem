                    ...
                    <c:forEach items="${rows}" var="r">
                        <tr>
                            ...
                            <td style="min-width: 320px;">
                                <c:set var="disableExcused" value="${empty r.requestStatus || r.excusedCount >= 3}"/>
                                <div class="btn-group" role="group" aria-label="Attendance">
                                    <input class="btn-check" type="radio" name="status_${r.enrollId}" id="st_${r.enrollId}_att" value="ATTENDED"
                                           ${empty r.status || r.status == 'ATTENDED' ? 'checked' : ''}>
                                    <label class="btn btn-outline-success btn-sm" for="st_${r.enrollId}_att">Attended</label>

                                    <input class="btn-check" type="radio" name="status_${r.enrollId}" id="st_${r.enrollId}_abs" value="ABSENT"
                                           ${r.status == 'ABSENT' ? 'checked' : ''}>
                                    <label class="btn btn-outline-danger btn-sm" for="st_${r.enrollId}_abs">Absent</label>

                                    <input class="btn-check" type="radio" name="status_${r.enrollId}" id="st_${r.enrollId}_exc" value="EXCUSED"
                                           ${r.status == 'EXCUSED' ? 'checked' : ''} ${disableExcused ? 'disabled' : ''}>
                                    <label class="btn btn-outline-warning btn-sm ${disableExcused ? 'disabled' : ''}" for="st_${r.enrollId}_exc">Excused</label>
                                </div>
                                <c:if test="${disableExcused}">
                                    <div class="form-text">Excused chỉ dùng khi có đơn xin phép và tối đa 3 buổi.</div>
                                </c:if>
                            </td>
                            <td>
