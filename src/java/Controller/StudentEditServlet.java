...
@WebServlet({"/admin/students/edit", "/consultant/students/edit"})
public class StudentEditServlet extends HttpServlet {
    private final StudentDAO studentDAO = new StudentDAO();
    private static final String TOKEN_KEY = "studentEditToken";
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            Student s = id > 0 ? studentDAO.findById(id) : null;
            if (s == null) {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            int id = parseInt(req.getParameter("studentId"), -1);
            Student existing = id > 0 ? studentDAO.findById(id) : null;
            if (existing == null) {
                resp.sendRedirect(req.getContextPath() + basePath(req) + "/students");
                return;
            }
            ...
            studentDAO.update(s);
            Flash.success(req, "Cập nhật học viên thành công.");
            resp.sendRedirect(req.getContextPath() + basePath(req) + "/students");
        } catch (Exception ex) {
            throw new ServletException(ex);
        }
